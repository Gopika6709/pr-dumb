...
static char read_raw_data(short *receive_buffer, unsigned char reg_addr, unsigned char byte_count, void *bus)
{
	char ret;
	...
	I2C_TARGET_DETAILS(gy_target, GY_521_ADDRESS, STANDARD_MODE, bus);
	reg_write.byte_count	= 1;
	reg_write.data_buffer	= &reg_addr;

	reg_read.byte_count	= byte_count; 
	reg_read.data_buffer	= &raw_data_buf[0];
...
char gy_init(unsigned char ad0_state, void *bus)
{
	...
	wakeup_write_transfer.byte_count	= 2;
	wakeup_write_transfer.data_buffer	= &wakeup_mng_reg[0];
	I2C_TARGET_DETAILS(gy_target, ADDRESS(ad0_state), STANDARD_MODE, bus);
...
char temperature_read(float *temperature_buffer, void *bus)
{
	...
	I2C_TARGET_DETAILS(gy_target, GY_521_ADDRESS, STANDARD_MODE, bus);
	temperature_reg				= TEMP_OUT; /* higher byte [15:8] */
